...
export default {
  ...
  created() {
    if (!this.disabled) {
      this.init();
      this.$root.$on('coediting-set-lock', this.setLock);
      this.$root.$on('coediting-remove-revision', this.removeRevision);
    }
  },
  beforeDestroy() {
    this.$root.$off('coediting-set-lock', this.setLock);
    this.$root.$off('coediting-remove-revision', this.removeRevision);
